...
#         self.volume_slider.configure(command=self.set_volume)  # Виправлено тут
...
class MP3PlayerApp(GUI):
    def __init__(self, root):
        ...
        self.loop_button.configure(command=self.toggle_loop)
        self.load_button.configure(command=self.load_music)
        self.delete_button.configure(command=self.delete_track)  # Додано кнопку видалення
    ...
    def play_music(self):
        ...
        if self.is_paused:
            ...
            # Додаємо обробку завершення треку
            if self.is_looping:
                pygame.mixer.music.set_endevent(pygame.USEREVENT)  # Встановлюємо подію завершення
                self.root.bind(pygame.USEREVENT, self.next_track_loop)  # Обробка події

        self.is_playing = True
        threading.Thread(target=self.update_time_display, daemon=True).start()

    def next_track_loop(self, event=None):
        """Метод для переходу до наступного треку з зацикленням"""
        if self.is_looping:
            self.next_track()  # Переходимо до наступного треку
            self.play_music()  # Відтворюємо його
    ...
    def next_track(self):
        if not self.track_list:
            return
        self.current_track = (self.current_track + 1) % len(self.track_list)  # Зациклюємо плейлист
        self.display_track_info()
    ...
    def delete_track(self):
        """Видалення вибраного треку з плейлисту"""
        if not self.track_list:
            return

        # Видаляємо поточний трек
        if self.current_track < len(self.track_list):
            del self.track_list[self.current_track]

            # Оновлюємо індекс поточного треку
            if self.current_track >= len(self.track_list):
                self.current_track = max(0, len(self.track_list) - 1)

            # Оновлюємо інтерфейс
            self.display_track_info()
            self.update_track_list()

            # Зупиняємо відтворення, якщо список порожній
            if not self.track_list:
